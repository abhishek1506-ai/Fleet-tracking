...
buildTableData(activeAssignments);
function buildTableData(data) {
    var table = document.getElementById("myTable");
    for (let i = 0; i < data.length; i++) {
        var row = `<tr>
                  <td id="${data[i].FleetID}" onclick="getFleetPage(this)"><a href="javascript:void(0)">${data[i].AssignmentID}</a></td>
                  <td id="${data[i].FleetID}" onclick="getFleetPage(this)"> <a href="javascript:void(0)">${data[i].FleetID}</a></td>
                  <td> ${data[i].Status}</td>
                  <td> ${data[i].ETA}</td>
                  <td> ${data[i].Driver}</td>        
              </tr>`
...
function getFleetPage(fleet) {
    sessionStorage.setItem("fleetId", fleet.id);
    window.location.href = 'fleettracking-two.html';
}
// Line chart
...
function assignTask(){
    alert("Fleet has beeen assigned");
}